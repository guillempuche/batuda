...
describe('ResearchService, the attributes a run is asked for', () => {
	...
	describe('when a run that reaches extraction declared no attributes', () => {
		it('should be handed a shape with no place to put attribute values', async () => {
			// GIVEN a run anchored on a page the fiber fetches, so extraction is
			// actually reached, and nothing declared
			...
			// THEN extraction happened, and no shape it was handed held an attribute
			// list — there is nothing there for the model to fill in and invent
			expect(TERMINAL.has(status)).toBe(true)
			expect(extractionSchemas.length).toBeGreaterThan(0)
			expect(extractionSchemas.some(hasAttributesField)).toBe(false)
		}, 60_000)
	})

	describe('when a run that reaches extraction declared an attribute', () => {
		it('should be handed a shape that has a place for the values', async () => {
			// GIVEN the same anchored run with one declaration
			extractionSchemas.length = 0
