...
const ManageCompanyChannels = Tool.make('manage_company_channels', {
	description:
		"The ways of reaching a company — its mailboxes, phones, website, social handles. A company can hold several of a kind, which is the point of this tool: `update_company` writes one email and one phone, and a firm with an orders mailbox, an accounts mailbox and a switchboard needs all of them kept apart. Give each one a `label` in the words somebody would actually use — 'orders', 'accounts', 'Girona shop' — because two addresses with no labels are indistinguishable a month later. Pass `site_id` to hang the channel off one branch instead of the company as a whole. action: 'list' (all of them; add site_id to see one branch's), 'add' (kind plus value, and a label whenever there is more than one of that kind), 'update' (by channel_id, only the fields to change), 'remove' (by channel_id), 'vouch' (by channel_id, email only). An address the company already holds is refused rather than merged, so correcting one onto another means removing the spare instead. 'vouch' is the way to get a held-back send out: it records that a person stands behind the address, so send_email stops asking about it, while leaving what a deliverability check found on file. Pass `note` to say what that rests on. It is refused on an address that hard-bounced or reported spam, which no vouch lifts. kind is open — email, phone, linkedin, instagram, website, x, bluesky, … — and `is_primary` marks the one to use when nothing says otherwise; the primary email is the address mail is sent to, and removing it hands that over to the oldest one left of the same kind.",
	parameters: Schema.Struct({
