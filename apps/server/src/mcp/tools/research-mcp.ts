...
const CONTEXT_SHAPE_HINT =
	'Expected context = { subjects?: [{ table, id }], selector?: { table: "companies", filter: { status?, industry?, country?, tags? } }, hints?: { language?, recency_days?, country?, place?, min_employees?, max_employees? } }. hints.country is an ISO 3166-1 alpha-2 code ("US"); hints.place is the place in words ("Baltimore metro"). A place written in words never sets the country.'
...
const StartResearch = Tool.make('start_research', {
	description:
		"Start a research run; returns {_tag:'started', id, status, applied_instructions, poll_after_ms?} immediately — then call get_research for results. Fresh research takes many minutes — a wide search legitimately keeps working for tens of minutes, and the server, not you, ends one that overruns — so status comes back 'queued' and poll_after_ms says how many milliseconds to wait before the first check; unless the same question was answered before, in which case status is already 'succeeded' and poll_after_ms is absent, meaning the findings are ready and there is nothing to wait for. " +
		SCHEMA_GUIDANCE +
		' applied_instructions lists the instruction templates that shaped the run. The user\'s default research instructions apply automatically; pass `stack` (a named stack, by name or id) to run a specific saved stack, and/or `instructions` (template names or ids) to layer extra templates after it for this run. An unknown or ambiguous `stack`/`instructions` ref returns {_tag:\'instruction_clarification\'} with candidates instead of starting. A run about a company already on file names it as `context.subjects: [{ table: "companies", id }]` — that link is what ties the run, its proposed updates and its attribute values to the row, so a run started without it can never be applied to the company. A `context.selector` — shaped `{ table: "companies", filter: { status?, industry?, country?, tags? } }` — researches every matching company (one run each); without `confirm:true` it returns {_tag:\'confirm_required\', subject_count, estimated_cost_cents} first so you can preview the scale — re-submit with confirm:true to launch (or narrow the filter). A malformed context returns {_tag:\'invalid_context\', error} without starting a run. When looking for NEW companies rather than filling in one already on file (`schema_name: "prospect_scan_v1"` with no `context.subjects`), pass what you are filtering on as fields, not only as words in the query: `context.hints.country` as an ISO 3166-1 alpha-2 code ("US"), `context.hints.place` for the place in words ("Baltimore metro"), and `min_employees`/`max_employees` for a size band. Only those fields are checked against what comes back; a requirement stated only in the query text is a request to the searcher, not a filter. If the user states a new standing preference, save it with manage_instructions.',
...
const GetResearch = Tool.make('get_research', {
	description:
		'Get the current state of a research run. Returns status, progressSteps, poll_after_ms, findings (if complete), cost, source_count, and applied_instructions — the instruction templates that shaped the run. When the stack the run was made with declares attributes, `findings.attributes` (on a scan, `attributes` on each company row) carries the value under each key, with `attributes.evidence.<key>` naming the page and the words it was read from; hand them on with create_companies or update_company as `attributes: { <key>: { value, source_id, quote } }` beside `research_id` set to the id of this run, so they are recorded as coming from the run. progressSteps counts the rounds of work the run has got through: null until the first round finishes, then climbing as the run works. It sits unchanged for minutes at a time by design — a late round scrapes and searches every gap it is still closing, and writing up the brief does not move it at all — so a count that is not moving is a slow run, not a stuck one. Keep polling: a run that really is stuck is failed by the server on its own deadline, so a stalled count is never a reason to call cancel_research. poll_after_ms is how many milliseconds to wait before calling this again; it is absent once the run has ended, which means stop calling. Two heavy fields are omitted by default to keep the response small: the full instruction-template text, and the list of pages the run read — a wide scan reads well over a hundred, which alone can push the answer past what a tool result may hold. `source_count` always says how many there were; pass include:[\'sources\'] for the list itself, and include:[\'instruction_segments\'] for the template text. Every field on a finding holds its value directly — `"website": "https://…"`, never a wrapper you have to open. A person\'s `role` is their title as the page writes it, in the page\'s own language, and the CRM keeps those words; on a `company_enrichment_v1` run, when that language is not English, an English rendering sits beside it as `role_gloss`. Where the run recorded which page a value came from, that sits in an `evidence` map beside the fields, keyed by the field it belongs to: `"evidence": {"website": {"source_id": "…", "quote": "…", "confidence": 1}}`. A field missing from `evidence` is one the run did not tie to a page, which is not a reason to distrust the value — most fields were never tied to one. A company on a scan\'s list may carry `marks` — findings the run established about that row rather than about the company\'s own business. `outside_requested_place` means the evidence puts it somewhere other than the area the run was asked about, with the run\'s reason in `outside_place_reason`: say so before adding such a company, and never record it as verified on this run\'s word alone. `existence_unconfirmed` means the run could not establish the company is real and trading, with what was missing in `existence_reason` — `no_own_site`, `one_website` and `no_sources` are findings about the company, while `budget_exhausted`, `deadline_reached` and `checker_unavailable` mean the run never got to check. A row without that mark is one the run stopped doubting, which speaks to the company being real and says nothing about where it is; `websites_seen` is how many independent websites named it, on marked and unmarked rows alike. `quality.place` says the same thing for the list as a whole — of the rows that said enough to be held to the area, how many were placed inside it, outside it, and read without being placed either way. It is absent when the run named no area. Read it before trusting a place-scoped list: a run that placed nobody inside the area asked for is the one case that also comes back as `succeeded_low_confidence`.',
	parameters: Schema.Struct({
