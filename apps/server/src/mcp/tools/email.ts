...
const SendEmail = Tool.make('send_email', {
	description:
		'Send a new email. The body is a structured block tree (paragraph / heading / list / quote / divider / image) — not raw html/text. Omit mailbox_id to use the calling member’s primary mailbox in the active org. Attachments reference staging_ids returned by stage_email_attachment; set inline=true for cid-referenced inline images. Before composing, read the member’s standing email instructions (writing style, sign-off, do/don’t rules) from the batuda://instructions/email resource and write the body to follow them. Returns {_tag:"sent"} on success or {_tag:"suppressed"} if a recipient is suppressed. Set skip_footer=true to omit the mailbox default footer.',
...
const ListEmailThreads = Tool.make('list_email_threads', {
	description:
		'List email threads with filters. Returns an envelope {items, total, limit, offset}. Each item carries message_count, last_message_at, last_message_direction, last_inbound_at, is_unread, and the linked mailbox {email, displayName, purpose}. Supports search by subject (query), status (open/closed/archived), and mailbox purpose (human/agent/shared). Default limit is 100.',
	parameters: Schema.Struct({
		mailbox_id: Schema.optional(Schema.String),
...
const GetEmailThread = Tool.make('get_email_thread', {
	description:
		'Get a full email thread with all messages from the provider. Each message is enriched with deliverability state (status, bounce_type) from messages.',
	parameters: Schema.Struct({
		thread_id: Schema.String,
	}),
...
// ── Mailbox management ─────────────────────────────────────────────
// Mailboxes are owned by an (organization, user) pair. Each row stores its
...
// mailboxes belong to a team member, agent mailboxes are AI-only, shared
// mailboxes surface in both.

const ImapSecurity = Schema.Literals(['tls', 'starttls', 'plain'])
const SmtpSecurity = Schema.Literals(['tls', 'starttls', 'plain'])

const ListEmailMailboxes = Tool.make('list_email_mailboxes', {
	description:
		'List mailbox rows visible to the calling member in the active organization. Each row carries purpose (human/agent/shared), ownerUserId, isDefault, active flag, IMAP/SMTP transport hosts, and grant_status. Filter by purpose, active flag, or owner. Private mailboxes belonging to other members are hidden automatically.',
	parameters: Schema.Struct({
		purpose: Schema.optional(MailboxPurpose),
	...
	.annotate(Tool.Title, 'List Email Mailboxes')
	.annotate(Tool.Readonly, true)
	.annotate(Tool.Destructive, false)
	.annotate(Tool.OpenWorld, false)
...
const GetEmailMailboxStatus = Tool.make('get_email_mailbox_status', {
	description:
		'Return the calling member’s primary mailbox in the active organization, if any. Use before send_email/create_email_draft to confirm the user has a default mailbox set; if hasDefault=false the UI should prompt them to connect a mailbox first.',
	success: Schema.Struct({
	...
	.annotate(Tool.Title, 'Get Email Mailbox Status')
	.annotate(Tool.Readonly, true)
	.annotate(Tool.Destructive, false)
	.annotate(Tool.OpenWorld, false)

const CreateEmailMailbox = Tool.make('create_email_mailbox', {
	description:
	...
	.annotate(Tool.Title, 'Create Email Mailbox')
	.annotate(Tool.Destructive, false)
	.annotate(Tool.OpenWorld, true)

const UpdateEmailMailbox = Tool.make('update_email_mailbox', {
	description:
		'Update fields on an existing mailbox in the active organization — display name, purpose, ownership, default flag, privacy, transport configuration, or credentials. Pass only the fields to change; password (if provided) is re-encrypted with a per-row HKDF subkey. Flipping `active=false` hides the mailbox from composers and stops the IMAP worker, but preserves historical thread links.',
	parameters: Schema.Struct({
		id: Schema.String,
	...
	.annotate(Tool.Title, 'Update Email Mailbox')
	.annotate(Tool.Destructive, false)
	.annotate(Tool.Idempotent, true)
	.annotate(Tool.OpenWorld, false)

const TestEmailMailbox = Tool.make('test_email_mailbox', {
	description:
		'Re-probe a stored mailbox: decrypts the saved credentials and runs a real IMAP LOGIN + SMTP check against the configured hosts, then updates grant_status / grant_last_error / grant_last_seen_at and returns the refreshed mailbox row. Use after changing a password (for example switching to an app-specific password) to confirm the mailbox now connects.',
	parameters: Schema.Struct({
		id: Schema.String,
	}),
	success: Schema.Unknown,
	...
	.annotate(Tool.Title, 'Test Email Mailbox')
	.annotate(Tool.Destructive, false)
	.annotate(Tool.Idempotent, true)
	.annotate(Tool.OpenWorld, true)

const DeleteEmailMailbox = Tool.make('delete_email_mailbox', {
	description:
		'Soft-delete an mailbox: sets active=false and is_default=false so the IMAP worker stops syncing it and composers stop offering it, while preserving historical messages and thread links. The row is not removed; use update_email_mailbox with active=true to restore.',
	parameters: Schema.Struct({
		id: Schema.String,
	}),
	success: Schema.Unknown,
	...
	.annotate(Tool.Title, 'Delete Email Mailbox')
	.annotate(Tool.Destructive, true)
	.annotate(Tool.Idempotent, true)
	.annotate(Tool.OpenWorld, false)

const SetPrimaryEmailMailbox = Tool.make('set_primary_email_mailbox', {
	description:
		'Promote one of the calling member’s human mailboxes to is_default=true (their primary From identity in the active organization). Clears the previous primary in the same (owner, purpose) bucket. Rejects shared mailboxes, inactive mailboxes, and mailboxes owned by another member.',
	parameters: Schema.Struct({
		id: Schema.String,
	}),
	success: Schema.Unknown,
	...
	.annotate(Tool.Title, 'Set Primary Email Mailbox')
	.annotate(Tool.Destructive, false)
	.annotate(Tool.Idempotent, true)
	.annotate(Tool.OpenWorld, false)
...
const ListEmailDrafts = Tool.make('list_email_drafts', {
	description:
		'List drafts for a specific mailbox. Returns draft metadata (no body). If mailbox_id is omitted, lists across all active mailboxes.',
	parameters: Schema.Struct({
		mailbox_id: Schema.optional(Schema.String),
	}),
	success: ListResult(Schema.Unknown),
...
const GetEmailDraft = Tool.make('get_email_draft', {
	description:
		'Get a single draft by id within an mailbox. Returns full draft contents including body_json. Returns null if no matching draft exists in the mailbox (or the draft belongs to a different mailbox).',
	parameters: Schema.Struct({
		mailbox_id: Schema.String,
...
const DiscardStagedEmailAttachment = Tool.make(
	...
	{
		description:
			"Permanently discard a staged email attachment (drops the row from email_attachment_staging). staging_id must belong to the supplied mailbox_id; mismatches are rejected so a tenant can't discard another tenant's staging row.",
		parameters: Schema.Struct({
			mailbox_id: Schema.String,
			staging_id: Schema.String,
		}),
...
const ListMailboxFooters = Tool.make('list_mailbox_footers', {
	description:
		'List all footers configured for an mailbox. The default footer is automatically appended to outbound emails unless skipFooter is set.',
	parameters: Schema.Struct({
		mailbox_id: Schema.String,
	}),
	success: ListResult(Schema.Unknown),
	...
	.annotate(Tool.Title, 'List Mailbox Footers')
	.annotate(Tool.Readonly, true)
	.annotate(Tool.Destructive, false)
	.annotate(Tool.OpenWorld, false)

const GetMailboxFooter = Tool.make('get_mailbox_footer', {
	description:
		'Get a single mailbox footer by id. Returns name, body_json (block tree), is_default, and timestamps. Org-scope is enforced by RLS — a footer belonging to another org returns NotFound.',
	parameters: Schema.Struct({
		id: Schema.String,
	}),
	success: Schema.Unknown,
	...
	.annotate(Tool.Title, 'Get Mailbox Footer')
	.annotate(Tool.Readonly, true)
	.annotate(Tool.Destructive, false)
	.annotate(Tool.OpenWorld, false)

const ManageMailboxFooter = Tool.make('manage_mailbox_footer', {
	description:
		'Manage an mailbox footer (the block tree appended to outbound emails). action=create adds a footer (body_json is the EmailBlocks shape used by send_email; is_default=true makes it the auto-appended default, one per mailbox); update changes name/body_json/is_default on footer_id (is_default=true atomically clears the prior default); delete permanently removes footer_id.',
	parameters: Schema.Struct({
	...
	.annotate(Tool.Title, 'Manage Mailbox Footer')
	.annotate(Tool.Destructive, true)
	.annotate(Tool.OpenWorld, false)
...
export const EmailTools = Toolkit.make(
	...
	GetEmailMailboxStatus,
	CreateEmailMailbox,
	UpdateEmailMailbox,
	TestEmailMailbox,
	DeleteEmailMailbox,
	SetPrimaryEmailMailbox,
	ManageEmailDraft,
	ListEmailDrafts,
	GetEmailDraft,
	ListMailboxFooters,
...
export const EmailHandlersLive = EmailTools.toLayer(
	Effect.gen(function* () {
		...
		const threadSendState = (threadLinkId: string, orgId: string) =>
			Effect.gen(function* () {
				const links = yield* sql<{
					...
					SELECT external_thread_id, contact_id FROM conversations
		...
		return {
			...
			discard_staged_email_attachment: ({ mailbox_id, staging_id }) =>
				staging.discard(mailbox_id, staging_id).pipe(
					Effect.map(() => ({ status: 'discarded' as const })),
					Effect.orDie,
				),
			list_email_mailboxes: params =>
				svc
					.listLocalMailboxes({
			...
			get_email_mailbox_status: () => svc.mailboxStatus(),
			create_email_mailbox: params =>
				svc
					.createMailbox({
						email: params.email,
					...
					.pipe(Effect.orDie),
			update_email_mailbox: params =>
				svc
					.updateMailbox(params.id, {
						...(params.display_name !== undefined && {
							displayName: params.display_name,
						}),
						...(params.purpose !== undefined && {
			...
			test_email_mailbox: params =>
				svc.testMailbox(params.id).pipe(
					Effect.catchTag('NotFound', e => Effect.die(e)),
					Effect.orDie,
				),
			delete_email_mailbox: params =>
				svc.deleteMailbox(params.id).pipe(
					Effect.catchTag('NotFound', e => Effect.die(e)),
					Effect.orDie,
				),
			set_primary_email_mailbox: params =>
				svc.setPrimaryMailbox(params.id).pipe(Effect.orDie),
			manage_email_draft: params => {
				...
				switch (params.action) {
					...
					case 'update':
						...
						return svc
							.updateDraft(params.mailbox_id, params.draft_id, fields)
							.pipe(Effect.orDie)
					case 'send':
						if (params.draft_id === undefined)
							return dieMissing('draft_id is required to send a draft')
						return svc.sendDraft(params.mailbox_id, params.draft_id).pipe(
							Effect.map(r => ({
								_tag: 'sent' as const,
								messageId: r.messageId,
								threadId: r.threadId,
					...
					case 'delete':
						...
						return svc
							.deleteDraft(params.mailbox_id, params.draft_id)
							.pipe(Effect.orDie)
				}
			},
			list_email_drafts: params =>
				svc
					.listDrafts(params.mailbox_id)
					.pipe(Effect.orDie, Effect.map(toItems)),
			get_email_draft: ({ mailbox_id, draft_id }) =>
				svc.getDraft(mailbox_id, draft_id).pipe(Effect.orDie),
			list_mailbox_footers: params =>
				svc.listFooters(params.mailbox_id).pipe(Effect.map(toItems)),
			get_mailbox_footer: ({ id }) => svc.getFooter(id).pipe(Effect.orDie),
			manage_mailbox_footer: params => {
				switch (params.action) {
					case 'create':
						if (
							params.mailbox_id === undefined ||
							...
							return dieMissing(
								'mailbox_id, name and body_json are required to create a footer',
							)
						return svc
							.createFooter({
								mailboxId: params.mailbox_id,
