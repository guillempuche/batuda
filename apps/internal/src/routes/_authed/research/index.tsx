...
/**
 * Everything the inbox shows above the fold — the pending proposals, the
 * paid-lookup queue, the runs needing attention, the run count and this
 * month's spend — so the whole screen paints on first render.
 */
function loadInboxOnServer(client: BatudaApiServerClient) {
...
export const Route = createFileRoute('/_authed/research/')({
	validateSearch: validateSearchWith({ dlg: researchDlgSchema }),
	loader: () =>
		handOverFromServer({
			label: 'ResearchInboxLoader',
			empty: { dehydrated: [] },
			fetch: loadInboxOnServer,
			handOver: data => {
				// Whatever arrived is handed over; a part that failed is left for the
				// browser to ask for again.
				...
				if (data.runCount) {
					dehydrated.push(
						dehydrateAtom(
							inboxRunCountAtom(),
							AsyncResult.success(data.runCount),
						),
					)
				}
				if (data.spend) {
