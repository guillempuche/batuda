...
Read it as "how much of this list is worth a second look", not "how many companies came twice". Three live re-runs of the French market row bear that out: the strict figure read 0% every time, and the loose one read 13%, 3% and 2% — one genuine repeat in each pass, plus, in the first, a row called `D'ASCENSEURS` that is a cut-off fragment rather than a company name and whose two words sit inside every lift company on the list. A list carrying junk rows reads higher here, and the row a reader then looks at is worth striking out either way.
