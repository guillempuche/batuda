		...
		make: Effect.gen(function* () {
			...
			const runFiber = (researchId: string, userId: string) =>
				Effect.gen(function* () {
					...
					// A run that declared no attributes is handed the same shape without
					// the attribute list: a field that is offered gets filled, and every
					// entry such a run returns is dropped again as undeclared.
					const outputSchema = extractionSchemaFor(
						schemaName,
						runAttributes.length > 0,
					)
					if (!outputSchema) {
						yield* sql`
							UPDATE research_runs
