...
// The one field on its way out: `current_tools` is now an attribute an
// organisation declares (the migration turns the old column into one), and
// the enrichment schema stops asking for it in the change that fills
// attributes from evidence. Until then it is the one name both sides use.
const LEAVING_THE_SCHEMA: ReadonlySet<string> = new Set(['current_tools'])
	...
	describe('when an organisation declares an attribute', () => {
		it('should never be able to take a name a findings schema already uses', () => {
			...
			const unreserved = [...used].filter(
				name =>
					!ATTRIBUTE_RESERVED_KEYS.has(name) && !LEAVING_THE_SCHEMA.has(name),
			)
			expect(unreserved).toEqual([])
		})

		it('should stop excusing a name once no schema asks for it', () => {
			// GIVEN the names the schemas use today
			const used = new Set<string>()
			for (const schema of Object.values(schemaRegistry))
				for (const name of propertyNames(Tool.getJsonSchemaFromSchema(schema)))
					used.add(name)

			// THEN every excused name is still one a schema asks for, so the excuse
			// goes when the field does rather than outliving it
			for (const name of LEAVING_THE_SCHEMA) expect(used.has(name)).toBe(true)
		})
	})
})
