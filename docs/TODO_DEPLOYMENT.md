...
- [ ] Canonical pnpm v11 deploy — drop `forceLegacyDeploy` + `@effect/*` overrides in `pnpm-workspace.yaml` and switch to `injectWorkspacePackages: true` with per-package `dependenciesMeta` injection. Attempted in PR #18 (closed); apps/internal's vite build fails on `linux-x64` CI with `Rolldown failed to resolve "@batuda/ui/blocks"` against the injected `@batuda/controllers/src/routes/pages.ts` copy, but the same lockfile builds cleanly on `darwin-arm64`. Defer until a Linux repro is in hand; current legacy + overrides setup is stable and shipped (api.batuda.co healthy)
