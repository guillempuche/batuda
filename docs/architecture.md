...
The **account brief** is one shared page that every side rewrites. A person, an agent over MCP, and a run's apply all replace what is there, so the last writer's text is what the brief says — there is no ownership, no append-only side, and no record of who wrote which version. Nothing keeps a previous version either, so whoever writes should read first and carry over what is still worth keeping. An apply only touches the brief when the run actually produced one, so a run that found nothing to say leaves a person's notes untouched.
