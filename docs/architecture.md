...
A tool call the model writes with argument names the tool does not have comes back to it as a tool result naming the tool and what it really accepts, the same way a dead page does, so the round keeps whatever its other calls found and the model writes the call properly next round. A vendor that refuses such a call at its own API returns no tool call to answer, so that one is corrected after the round instead. Either way the model is told what was wrong: asking the same question again only gets the same bad call, on every vendor slot, so the retries are the safety net rather than the mechanism.
...
```
  ...
  Phase 2 · structured extraction
      the request fenced again; each declared attribute named by key, kind,
          ...
          never becomes the CRM title; the title is held to the page's own
          line (a reworded quote backs nothing), a word the model inflected
          its own way is put back to the page's, a label beside a name
          ("Contact", "Tel") or a headline of several posts is not a title,
          a title read off the company's own site outranks a directory's,
          and a name that is an address, a number or a bare first name is
      ...
      guard chain — named links, run in the order they are written; a link
          ...
          organisation kind · quote text · citations · scalars · attributes
          websites · contact entity
          value provenance · fit evidence · vocabulary · applicability
          discovered-existing · scan evidence · prospect criteria
          prospect dedupe · shared handles · network · place · unconfirmed mark
...
How a run fills them: the searching pass is told what each attribute means — its label, unit and description, each fenced like a standing instruction — and the pass that writes the answer is told only the key, the kind, the unit and the choice words — the short, capped parts — so an admin's sentences never sit beside the rules on what counts as evidence. The values come back as a list of entries, become a map keyed by attribute before any guard reads them, and are held by a guard of their own to their page, the words quoted for them and their kind: the quote has to be the page's own words run for run (a remark the model writes about the page, "no se indica", backs nothing, so "not stated" is never stored as a no or a zero), a number the quote states digit for digit, a choice from the declared words, a date whose year the quote names, a yes or a no, text in the quote's own words (a remark the model adds after it in brackets comes off, and a value the quote does not write is dropped as `value_not_quoted`); a date the value is said to hold as of stays only when the quote names its year. A run about one company whose only news is its attribute values gets a proposal of its own, an update with no fields, so a person applies them like any other change; it is added once the last gap round has folded what it read into the findings, since an attribute is as often read in a round as in the first pass and the guard chain runs before the fold; what a round hands back to the CRM — its proposed updates, its paid follow-ups, the rows it found already on file — rides the fold with the fields it filled, an entry the list already holds left as it is. A run whose organisation declared none is handed the same extraction shape with the attribute list left out altogether, so there is no field there to fill and nothing to invent. A gap round searches for a declared attribute a company lacks the way it searches for a missing website, by the attribute's label. The declarations ride on the run row, so a run restarted from its row asks for the same facts, and their fingerprint rides in the research cache key, so an edited declaration never serves a cached run made for the old wording.

Where people meet them: an admin declares and retires them on the organisation's research templates page, stack by stack, and flips the switch on the stack's editor; a company page shows them in the About section, editable by kind, with the page, quote and run under a value a research run set; the companies list and its board can be narrowed by one of them; a run page lists what the run found, labelled through the declarations, and the review of a proposed change says which of those values land with it. Every surface reads the declarations once, org-wide; the pages that show values are handed them with the page by their loader, and every surface tells "not known yet" apart from "none declared", so a value is never offered for clearing while the answer is still on its way.
...
Three safety invariants hold regardless of budget: the agent never mutates a CRM row (it proposes; a human applies, under optimistic concurrency); a claim reaches the findings only with a citation that resolves to a page the run actually reached and quotes words that page wrote — the quote is tidied first (the model's own quote marks, a search result's "[source: host]" marker and a snippet's joiners come off, and a page quoted several times over is cited once), an opaque page id or a bare host is put back to the page's address, and the citations guard drops any citation that names no fetched page, takes the words off one that quotes words on none (the page was read; a person named on it in a caption stays a person on it once the model's remark is gone), and the run carries on with what is left, so an uncited claim costs the claim, never the run — where a page it opened is archived, while one it only saw named in a search result is recorded and citable without a stored copy; and paid calls are metered idempotently so a retried fiber is never double-charged.
