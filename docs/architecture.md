...
The **account brief** is one shared page that every side rewrites. A person, an agent over MCP, and a run's apply all replace what is there, so the last writer's text is what the brief says — there is no ownership, no append-only side, and no record of who wrote which version. Nothing keeps a previous version either, so whoever writes should read first and carry over what is still worth keeping. Two things hold an apply back from it: the run has to have actually produced a brief, and a person has to have set the apply going. The server applying on its own writes neither the brief nor the fit judgement — see below.
