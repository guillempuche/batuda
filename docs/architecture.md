...
Both entry points accept `--env local|cloud`. In cloud mode the loader fetches non-secret GitHub Actions Variables via `gh variable list --env production` and merges them into `process.env` between the local `.env` baseline and the user's `.env.cloud` (so `.env.cloud` only needs to carry secrets, which GitHub never exposes via the API).
