...
Every outbound path — `send`, `reply`, `sendDraft` — funnels through the private `recordOutbound` helper in `apps/server/src/services/email.ts`, which writes the `email_messages` row and the `email_sent` history entry inside one transaction. It then calls `claimLeadOnEmail` (`apps/server/src/services/company-lead-assignment.ts`): if the company has no `owner_id` it becomes the sender's, and if it is still at `prospect` it moves on, each recorded on the company's history.
Which stage it lands on depends on who started the conversation — `responded` when the company already wrote to us, `contacted` when this is us opening. The two are told apart by an `email_received` entry on the company's own history, which the worker writes only for a message that arrived and matched it, so our copies in the Sent folder cannot be mistaken for them answering. It matters because the stage only ever moves off `prospect` once: flattening both into `contacted` would lose the difference between a conversation and a cold approach for good.

The rule reads `owner_id IS NULL` and `status = 'prospect'`, **not** "has this company been emailed before". The mail worker files Sent-folder mail as `direction = 'outbound'`, so that second question is answerable — it is just the wrong one. A mailbox connected with years of history carries correspondence from before Batuda existed, and whether somebody wrote to a company back then says nothing about whether anybody is working the lead today. Who has taken it does.

Each write is a single conditional `UPDATE … RETURNING`, so two people emailing the same company at once cannot both win it: the second blocks on the first one's row lock, re-reads the committed row, and matches nothing. That lock costs nothing extra — recording the sent email bumps the same row's `last_email_at` a moment earlier in the same transaction, so the request already holds it. The stage write bumps `version`, the token a research proposal is checked against, so a proposal prepared before the send cannot put the old stage back; the owner write leaves it alone, since research cannot write an owner and bumping there would only invalidate proposals about unrelated fields. Neither write touches a company somebody has deleted. The owner write also refuses a sender who has left the organisation — the owner column has nothing in the database behind it, so that check is all that stops them — while the stage still moves, because an email went out whoever sent it. The claim runs in its own nested transaction (a savepoint) and swallows its own failures — by the time it runs the message has already left over SMTP, and losing the record of it would have somebody send it twice.

Who is sending is a required `actor` argument rather than an ambient `SessionContext` read, so a new way of sending has to say who it is. It carries `claimsLead` beside the id, because being named on a message and reaching out are different things: an agent under a shared API key is named but claims nothing, an RSVP reply names whoever answered without taking the lead, and a forwarded invitation names nobody because that path is never told who asked for it. An agent send skips the stage move as well as the owner write, so a company only an agent has emailed stays at `prospect`.
...
What stays here is the code-facing detail. Providers are selected at boot by `RESEARCH_PROVIDER_*` env vars with a comma-list fallback chain; `brave/search.ts` is the template adapter and [Adding a new research capability provider](#adding-a-new-research-capability-provider) below is the recipe. The eval harness runs from the CLI, never in production — `pnpm cli research eval` reports grounding accuracy, field precision and recall, and the wrong-company and empty rates, and `pnpm cli research probe` checks which candidate LLMs support the forced tool-calling and strict-JSON output the tiers need. [eval/README.md](../eval/README.md) is the manual for both: what kind of harness it is, how to run a pass and provide keys, how to write a golden row, and what the numbers can and cannot say.
