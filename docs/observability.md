...
| `research.tool_call_corrected` | A model was told a tool call it wrote would not be accepted |
...
The same shape serves the attributes an organisation declares on its companies. A company write or list refused over one carries `attribute.refused.reason` (`undeclared_key`, `wrong_kind`, `operator_not_for_kind`, …) beside its 400; a declaration write carries `attribute.declare.outcome`, since the code comes back in a successful body; and an apply that met a run's attribute values carries how many landed, were held back for a person's own value, or were dropped, as `research.apply.attributes_landed`, `_held`, `_dropped` and `_dropped_reasons` (`undeclared`, `wrong_kind`, `ungrounded`, or `unfetched` when the run cited a page it never opened). Inside a run, the guard that grades the values a model wrote puts `research.attributes.kept` and `research.attributes.dropped` on the run's span, and its `research.attributes.dropped` warning names the reasons (`undeclared`, `ungrounded`, `unquoted`, `unsupported`, `off_entity`, `wrong_kind`, `value_not_quoted` for a text value the quote does not write), and a run about one company that kept values logs `research.attributes.proposed` when it adds the proposal that carries them; `research.fields.dropped_unquoted` counts a headcount or a job title that arrived with no words behind it, `research.fields.dropped_quote_absent` a value whose quote appears nowhere in the evidence (for a job title, and for every attribute, the quote has to be the page's own words run for run, so a reworded line counts as absent), `research.fields.dropped_unsupported` one whose quote is real but does not state the value, `research.fields.dropped_not_title` a job title that was a label beside a name or a headline, and `research.contacts.wrong_entity` carries `dropped_not_person` for a contact whose name was an address, a number or a bare first name and `dropped_site_credit` for one a page credited with making the site; `research.quotes.tidied` says how many quotes lost a marker or a wrapping (`cleaned`), `research.citations.dropped` carries `stripped_quotes` for citations whose words are on no fetched page (the page stays, the words go) and `folded` for citations that said the same words of the same page as an earlier one, or no words of a page another citation quotes (also `research.citations.folded` on the span), and `research.prospects.shared_handles` counts a social profile taken off a scan row whose own pages never showed it; and a structured reply that ran past the most one reply may write leaves a `research.extraction.cut_off` warning and `research.extraction.compacted_retries` on the span, since it is asked for once more, shorter, rather than retried. A page whose main content read as a cookie notice is fetched once more whole and logs `research.scrape.refetched_full` with the `url`, `chars_main`, `chars_whole`, `whole_fetched` (false when the second fetch failed and the notice stood) and `kept_whole` (whether the fuller reading took the notice's place), so a site whose main-content extraction returns only its cookie notice shows up as one line rather than as a team page with nobody on it, and a site that trips the second fetch on every page shows it on every page. A reply cut off a second time is not lost: the part of it that arrived whole is kept, under a `research.extraction.salvaged` warning that says how much of the reply that was (`kept_chars` of `total_chars`) and the same name counted on the span, and the run's quality block carries `reply_cut: true` and reads as low confidence, since what it holds is what fitted in the reply rather than all the evidence held. Where losing the reply would lose the run — the first reading of a scan or of a contact search, which has no earlier list to fall back on — both cut-off replies are read and the fuller one ships, and a reply that fits the shape at no depth is read once more row by row, since one row written wrong fails every whole reading of it: the line then also carries `rows_kept` and `rows_left_out`. A rescue that finds nothing whole logs `research.extraction.salvage_empty` with `total_chars` (and counts the same name on the span) before the run fails, so a rescue that kept nothing is told apart from one that never ran. Every model call's span carries `llm.finish_reason` (why the model stopped), and one the vendor cut at the ceiling logs `llm.output_truncated`, so a ceiling set too low shows before it breaks a reply. A model that wrote a tool call with arguments the tool does not take is told so rather than asked again: usually as a tool result inside the round, which leaves no line of its own, and — when a vendor refused the call at its own API and there is no tool call to answer — as a correction after the round, which logs `research.tool_call_corrected` with the `tool` and the `provider` that refused it, so a vendor that keeps turning one tool's arguments away reads as a handful of lines rather than as runs that died for no visible reason. Codes and counts only — never a key's value, which is the tenant's data.
