...
Every loader that fetches ahead of the page is written the same way, with two helpers. `withServerApi` (`src/lib/batuda-api-server.ts`) runs one Effect program against the API as the visitor whose page is being rendered; loaders reach it through `import('#/lib/batuda-api-server')` so the server client never lands in the browser bundle. `handOverFromServer` (`src/lib/atom-hydration.ts`) is the envelope: nothing in the browser, the fetch on the server, a failed fetch logged and degraded to an empty handover, and the handover itself outside that safety net so a missing serialization key breaks the page instead of quietly turning into a refetch. A page that should give up rather than degrade names `notFoundWhen: isNotFoundError`, so a record that does not exist becomes a not-found page instead of an empty one that fetches again.

Auth is the exception: it goes through `authClient` (Better Auth), whose store lives in the browser only. Screens never read that store directly: the exported `authClient` has no store hooks, and the four hooks in `src/lib/auth-client.ts` (`useHydratedSession`, `useHydratedActiveOrganization`, `useHydratedListOrganizations`, `useHydratedActiveMember`) are the way to read it. The root route reads who is signed in and which organisation they are in on the server (`src/lib/server-identity.ts`, only the fields a first frame needs, never the session token) and hands that down with the page, so the org name and the owner names are right on the first frame on both sides; the hooks switch to the live store once it has an answer of its own (`serverFirst` in `src/lib/identity.ts`, with its own tests). `createServerFn` is used in exactly one place, `src/lib/server-cookie.ts`, for cookie access during SSR — not for reaching the API.
