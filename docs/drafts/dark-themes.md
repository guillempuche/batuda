...
Nothing outstanding. The contrast tables are now enforced by `pnpm --filter @batuda/ui check-contrast` on pre-push, and frontend.md carries a §Theming section describing the attribute, the preference/resolved split, and the surfaces the palette cannot reach.
...
- **Fixed: the CSS is no longer duplicated into `dist/`.** It used to be, and the export map preferred that copy, so editing `src/tokens.css` changed nothing on screen until the package was rebuilt *and* Vite restarted — a stale build silently beat source. This cost an hour during Slice 6: the theme blocks were correct, the attribute was on `<html>`, the selectors matched, and a stale copy was being served. The two files were byte-identical, so the copy bought nothing; the package now serves them straight from source and publishes them from there too.
