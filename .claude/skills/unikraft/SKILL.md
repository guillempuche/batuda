...
2. **FROM scratch runtime** - Copies only required binaries/libraries
3. **Manual library copying** - Must copy `ld-musl`, `libgcc_s`, `libstdc++`
4. **SSL certificates** - Required for HTTPS connections (Neon DB, etc.)
5. **Empty .env files** - Dotenv needs files to exist even if env vars come from `-e` flags
...
2. `--name` must be **omitted** (kraft auto-generates unique names so new instance can spin up while old exists)
