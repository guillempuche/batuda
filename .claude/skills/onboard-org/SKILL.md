...
Settle the details first (§0), then five steps in order: **rehearse → read up → create →
...
Before writing anything to production, spend a minute on who they are: open their website and
note what they sell, who they sell it to, and whether they work in one country or anywhere.
It costs one page read and it decides whether the handover in §7 sounds like it was written
for them or for somebody else.

This is reading a public page to write a better message. Nothing here is written to their
records, and nothing guessed leaves the message.
...
`https://api.batuda.co/mcp`, and the examples, written from what §2 turned up about their
market.
