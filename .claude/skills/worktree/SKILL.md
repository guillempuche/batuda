...
- **Committed changes after a rebase- or squash-merge**: the local branch is no longer an ancestor of `main`, because the merge rewrote the commits. `worktree done` asks GitHub whether a pull request for the branch was merged, and deletes it when one was. Failing that — no `gh`, signed out, nothing to reach — it falls back to comparing the branch against `main`, which answers for a plain rebase or squash but NOT once `main` has gained a commit touching the same files: the rebase folds that commit into them, so nothing on `main` is identical to the branch even though the work is there. When neither reading can say yes it refuses, says so, and leaves the branch alone; check the pull request before reaching for `--force`, which deletes without looking.
